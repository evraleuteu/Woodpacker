...
const EXTRACTION_URL =
  process.env.DOCLING_EXTRACTION_SERVICE_URL ??
...
async function proxy(
  ...
  // /api/docling/... -> http://docling-extraction-service:8001/...
  ...
  const upstream = await fetch(url, {
    ...
    return new Response(JSON.stringify({ error: `docling service unreachable: ${e}`, service: 'docling', url }), {
      status: 502,
      headers: { 'content-type': 'application/json' },
    }) as unknown as Response
  })

  // Guard for mocked Response in catch
  if (!upstream || typeof (upstream as Response).headers?.get !== 'function') {
    return NextResponse.json({ error: 'docling service unreachable' }, { status: 502 })
  }
