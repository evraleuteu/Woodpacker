...
// PyMuPDF Inspector — baseline text extraction (no OCR)
// Stateless: every request hits pymupdf-service:8003 with LAYOUT_FORCE_PROVIDER=pymupdf
// No fallback — benchmark validity requires the forced engine is actually used
const EXTRACTION_URL =
  process.env.PYMUPDF_SERVICE_URL ??
  'http://127.0.0.1:8003'
...
async function proxy(
  ...
  const upstream = await fetch(url, {
    ...
    return new Response(JSON.stringify({ error: `pymupdf service unreachable: ${e}`, service: 'pymupdf', url }), {
      status: 502,
  ...
  if (!upstream || typeof (upstream as Response).headers?.get !== 'function') {
    return NextResponse.json({ error: 'pymupdf service unreachable' }, { status: 502 })
  }
