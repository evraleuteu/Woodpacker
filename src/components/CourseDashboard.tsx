...
function KnowledgeGraph({ course }: { course: Course }) {
  ...
  return (
    <div className="rounded-xl border border-[rgba(250,248,245,0.06)] bg-[rgba(250,248,245,0.02)] overflow-x-auto">
      <div className="flex items-center gap-4 px-4 py-3 border-b border-[rgba(250,248,245,0.06)]">
        <span className="text-xs text-[#A8A29E]">Node color = difficulty</span>
        ...
        <span className="flex items-center gap-1.5 text-xs text-[#A8A29E] ml-auto">
      ...
      {steps.map((step, i) => (
        <div key={step.id} className="flex items-center gap-2 shrink-0">
          ...
          >
            ...
            <div className="text-xs font-medium text-[#FAF8F5] leading-snug">{step.title}</div>
          </div>
          {i < steps.length - 1 && (
  ...
  return (
    <div className="p-4 rounded-xl bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
      <div className="flex items-center gap-2 mb-1">
        <span className="text-sm font-semibold text-[#FAF8F5]">{item.term}</span>
      ...
      {item.definition && <div className="text-xs text-[#A8A29E] mb-2">{item.definition}</div>}
  ...
  return (
    <div className="p-4 rounded-xl bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <span className="text-sm font-semibold text-[#FAF8F5]">{rule.name}</span>
        <DifficultyBadge difficulty={rule.difficulty} />
      </div>
      {rule.explanation && <div className="text-xs text-[#A8A29E] mb-2">{rule.explanation}</div>}
...
function ExerciseList({ exercises, accent }: { exercises: Lesson['exercises']; accent?: string }) {
  ...
  return (
    <div className="space-y-2">
      {exercises.map((ex) => (
        <div key={ex.id} className="p-3 rounded-lg bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] uppercase tracking-wider text-[#6B7280] font-semibold border border-[rgba(250,248,245,0.1)] rounded px-1.5 py-0.5">
              {ex.type}
            </span>
            {accent && <span className="text-[10px] font-medium" style={{ color: accent }}>{accent}</span>}
          </div>
          <div className="text-xs text-[#FAF8F5]">{ex.prompt}</div>
          {ex.options && ex.options.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {ex.options.map((opt, i) => (
                <span key={i} className="text-[11px] text-[#A8A29E] bg-[rgba(250,248,245,0.03)] border border-[rgba(250,248,245,0.08)] rounded px-2 py-0.5">
                  {opt}
                </span>
              ))}
            </div>
      ...
      <div className="space-y-4">
          ...
          <div>
            ...
            <ul className="space-y-1">
              {lesson.objectives.map((o, i) => (
                <li key={i} className="text-xs text-[#FAF8F5] flex gap-2">
        ...
        {grammar.length > 0 && (
          <div>
            ...
            <div className="space-y-2">
              {grammar.map((g) => (
                <div key={g.id} className="p-3 rounded-lg bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
                  <div className="text-xs font-semibold text-[#FAF8F5] mb-0.5">{g.name}</div>
                  <div className="text-xs text-[#A8A29E]">{g.explanation}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {vocabulary.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {vocabulary.map((v) => (
                <span key={v.id} className="text-[11px] text-[#FAF8F5] bg-[rgba(16,185,129,0.08)] border border-[rgba(16,185,129,0.2)] rounded-full px-2.5 py-1">
                  {v.term}
                </span>
              ))}
            </div>
      ...
      <div className="space-y-4">
        {grammar.filter((g) => g.examples.length).length > 0 && (
          <div>
            ...
            {grammar.filter((g) => g.examples.length).map((g) => (
              <div key={g.id} className="mb-2 p-3 rounded-lg bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
                <div className="text-xs font-semibold text-[#FAF8F5] mb-1">{g.name}</div>
        ...
        {lesson.materials.reading && (
          <div>
            ...
            <div className="p-3 rounded-lg bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
              <div className="text-xs font-semibold text-[#FAF8F5] mb-1">{lesson.materials.reading.title}</div>
              <p className="text-xs text-[#A8A29E] leading-relaxed">{lesson.materials.reading.passage}</p>
            </div>
          </div>
        )}
        {lesson.materials.listening && (
          <div>
            ...
            <div className="p-3 rounded-lg bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
              <div className="text-xs font-semibold text-[#FAF8F5] mb-1">{lesson.materials.listening.title}</div>
              <p className="text-xs text-[#A8A29E] leading-relaxed">{lesson.materials.listening.transcript}</p>
            </div>
          </div>
        )}
        ...
        {engineTabs.map((t) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${
              tab === t.id
                ? 'text-white border-[rgba(16,185,129,0.4)] bg-[rgba(16,185,129,0.1)]'
                : 'text-[#A8A29E] border-transparent hover:text-[#FAF8F5] hover:bg-[rgba(250,248,245,0.04)]'
            }`}
          >
  ...
  return (
    <div className="space-y-3">
      {course.modules.map((module, mi) => {
        ...
        return (
          <div key={module.id} className="rounded-xl glass-card overflow-hidden">
            <button onClick={() => setOpenModule(open ? null : module.id)} className="w-full flex items-center gap-3 p-4 text-left">
              ...
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#059669] to-[#10B981] flex items-center justify-center text-[10px] font-bold shrink-0">
                M{mi + 1}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-[#FAF8F5]">{module.title}</div>
                <div className="text-xs text-[#A8A29E] truncate">{module.description}</div>
              </div>
              ...
              {open && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <div className="border-t border-[rgba(250,248,245,0.06)] px-4 py-2 space-y-2">
                    {module.lessons.map((lesson, li) => {
                      ...
                      return (
                        <div key={lesson.id} className="rounded-lg border border-[rgba(250,248,245,0.06)] bg-[rgba(250,248,245,0.02)] overflow-hidden">
                          ...
                          <AnimatePresence>
                            {lessonOpen && (
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-[rgba(250,248,245,0.06)]">
                    ...
                    {module.review && (
                      <div className="p-3 rounded-lg border border-[rgba(16,185,129,0.15)] bg-[rgba(16,185,129,0.03)] flex items-center gap-2">
                        <RefreshCw size={12} className="text-[#10B981]" />
                        <span className="text-xs text-[#A8A29E]">{module.review.title}</span>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start gap-4">
        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#059669] to-[#10B981] flex items-center justify-center shrink-0 ai-glow">
          <Sparkles size={20} className="text-white" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-xl font-bold text-[#FAF8F5]">{course.title}</h2>
            {course.mode === 'ai' ? (
              <span className="flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-[rgba(16,185,129,0.1)] border border-[rgba(16,185,129,0.3)] text-[#10B981]">
                <Sparkles size={10} /> AI Transformation
              </span>
            ) : (
              <span className="flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-[rgba(59,130,246,0.1)] border border-[rgba(59,130,246,0.3)] text-[#3B82F6]">
                <Cpu size={10} /> Local Extraction
              </span>
            )}
          </div>
          <p className="text-xs text-[#A8A29E] mt-0.5">{course.description}</p>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
        {statItems.map((stat) => (
          <div key={stat.label} className="text-center p-3 rounded-xl bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
            <div className="text-lg font-bold text-gradient">{stat.value}</div>
            <div className="text-[11px] text-[#A8A29E] mt-1 flex items-center justify-center gap-1">
        ...
        {tabs.map((t) => (
          ...
          >
            ...
            {t.id === 'duplicates' && course.duplicates.length > 0 && (
              <span className="text-[10px] bg-[rgba(245,158,11,0.15)] text-[#F59E0B] rounded-full px-1.5">{course.duplicates.length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
          ...
          {tab === 'vocabulary' && (
            <div>
              <div className="text-xs text-[#A8A29E] mb-3">{allVocabulary.length} unique terms across all lessons</div>
          ...
          {tab === 'grammar' && (
            <div>
              <div className="text-xs text-[#A8A29E] mb-3">{allGrammar.length} unique grammar rules across all lessons</div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                {allGrammar.map((g) => (
          ...
          {tab === 'duplicates' && (
            <div className="space-y-3">
              {course.duplicates.length === 0 && (
                <div className="p-6 text-center rounded-xl bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
                  <CheckCircle2 size={20} className="text-[#10B981] mx-auto mb-2" />
                  <div className="text-sm text-[#A8A29E]">No duplicate content detected across your materials.</div>
                </div>
              )}
              {course.duplicates.map((dup) => (
                <div key={dup.id} className="p-4 rounded-xl bg-[rgba(250,248,245,0.02)] border border-[rgba(250,248,245,0.06)]">
                  ...
                  <div className="text-xs text-[#A8A29E] mb-2">{dup.rationale}</div>
                  <div className="flex flex-wrap gap-1.5">
                    {dup.items.map((item, i) => (
                      <span
                        ...
                        className={`text-[11px] px-2 py-0.5 rounded-full border ${
                          item === dup.kept
                            ? 'text-[#10B981] border-[rgba(16,185,129,0.3)] bg-[rgba(16,185,129,0.08)]'
      ...
      <div className="flex items-center justify-between p-4 rounded-xl bg-[rgba(16,185,129,0.04)] border border-[rgba(16,185,129,0.12)]">
        <div className="flex items-center gap-3">
          <CheckCircle2 size={20} className="text-[#10B981]" />
          <div>
            <div className="text-sm font-medium text-[#FAF8F5]">Course saved locally</div>
            <div className="text-xs text-[#A8A29E]">Open the Woodpecker engine on any lesson to start learning.</div>
          </div>
        </div>
